...
import httpx
from citeurl import Citator, list_cites
...
citation_server = FastMCP("citations")
...
async def lookup_citation(
    ...
    Returns:
        dict: The opinion(s) that match the citation, or an error dict if the lookup fails.
...
def get_citator(template_path: Path | None = None) -> Citator:
    ...
    """
    return _get_citator_singleton(template_path)


@lru_cache(maxsize=1)
def _get_citator_singleton(template_path: Path | None = None) -> Citator:
    """Get or create the citeurl citator instance with custom citation templates.

    Returns:
        Citator: The singleton citeurl Citator instance.

    """
    if template_path is not None:
        logger.info(
            f"Creating citator with custom citation templates from {template_path}"
        )
        citator = Citator(yaml_paths=[str(template_path)])
    else:
        citator = Citator()
        logger.info("Created citator with default citation templates")
    return citator
...
async def parse_citation_with_citeurl(
    ...
    Returns:
        dict: A dictionary containing the parsed citation data,
    ...
    try:
        citator = get_citator(TEMPLATE_PATH)
...
async def enhanced_citation_lookup(
    ...
    Returns:
        dict: A dictionary containing the enhanced citation information.
