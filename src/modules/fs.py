...
def get_scoring_function(
    scoring: Union[str, Callable, None], top_p: float = 0.05
) -> Callable:
    """
...
class AutoFeatureSelection:
    ...
    def run_optuna_selection(
        ...
        # Run optimization
        study.optimize(
            fs_optuna, n_trials=n_trials, timeout=timeout, show_progress_bar=True
        )

        # Get selected features from best trial
        selected_features = [
            feat
            for feat, selected in zip(features, study.best_trial.params.values())
            if selected
        ]
    ...
    def run_kbest_selection(
        ...
        selected_features = [
            feat for feat, selected in zip(features, skb.get_support()) if selected
        ]
        ...
        ranking = dict(
            zip(
                features,
                [
                    sorted(scores.values(), reverse=True).index(score) + 1
    ...
    def run_model_based_selection(
        ...
        if importance_threshold is not None:
            selected_features = [
                feat for feat, score in scores.items() if score >= importance_threshold
            ]
        elif top_n is not None:
            selected_features = [
                feat
                for feat, _ in sorted(scores.items(), key=lambda x: x[1], reverse=True)[
                    :top_n
                ]
            ]
        else:
    ...
    def get_feature_intersection(
        ...
        if not all(method in self.results for method in methods):
            raise ValueError(
                f"Some methods not yet run. Available methods: {list(self.results.keys())}"
            )

        # Get sets of selected features for each method
        method_features = [
            set(self.results[method].selected_features) for method in methods
        ]

        # Return intersection
        return list(set.intersection(*method_features))

    def get_top_n_consistent(
        self, n: int = 10, methods: Optional[List[str]] = None
    ) -> List[str]:
        """
        ...
        for feature in self.all_features:
            rankings = [
                ranking.get(
                    feature, len(self.all_features)
                )  # Use worst rank if feature not in ranking
        ...
        return [
            feature
            for feature, _ in sorted(avg_rankings.items(), key=lambda x: x[1])[:n]
        ]
