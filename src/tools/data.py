...
class AnalystAgentDeps:
    ...
    def list_files_in_data_directory(self) -> list[str]:
        ...
        files = [
            f
            for f in files
            if not f.startswith(".") or f not in ["__pycache__", "node_modules", ".git"]
        ]
...
def get_eda_report(
    ctx: RunContext[AnalystAgentDeps], dataset_path: str, target: str | None = None
) -> str:
    ...
    try:
        ...
        raise ModelRetry(
            f"Dataset '{dataset_path}' not found in context. Please load the dataset using `load_huggingface_dataset` \
tool, or pick from the following datasets: {str(ctx.deps.list_files_in_data_directory())}"
    ...
    if target is not None and target not in df.columns:
        ...
        raise ModelRetry(
            f"Target column '{target}' not found in dataset. Please select from the following columns: {valid_columns}"
        )
...
data_tools = FunctionToolset(
    tools=[load_huggingface_dataset, get_eda_report], max_retries=5, id="data_toolset"
)
