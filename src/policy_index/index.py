...
class PolicyIndex:
    ...
    async def _meta_processor(
        ...
        Implements a self-healing pipeline inspired by v1's
        ``meta_processor`` / ``fix_incorrect_toc_with_retries``:

        1. Process the TOC via the selected *mode*.
        2. Validate and verify accuracy.
        3. If accuracy is perfect (1.0), return immediately.
        4. If accuracy > 0.6, run ``_fix_incorrect_with_retries`` (up to 3
           attempts) then re-verify.
        5. If accuracy is between 0 and 0.6, attempt a single fix pass.
           If the fix raises accuracy above the threshold, keep the result;
           otherwise fall back to a simpler processing mode.
        6. If all modes are exhausted, return whatever partial results exist
           rather than raising an exception (graceful degradation).
        ...
        # --- High accuracy (>0.6): fix with retries + re-verify ---
        if accuracy > 0.6 and incorrect:
            items, remaining = await self._fix_incorrect_with_retries(
                items, pages, incorrect, start_index, max_attempts=3
            )

            # Post-fix re-verification for observability
            post_accuracy, post_incorrect = await self._verify_toc(
            ...
            if post_incorrect:
                logger.warning(
                    "%d items still incorrect after fix retries and re-verification.",
                    len(post_incorrect),
                )

            return items

        # --- Low accuracy (0 < accuracy <= 0.6): try single fix pass ---
        if accuracy > 0 and incorrect:
            logger.info(
                "Low accuracy (%.2f%%). Attempting single fix pass before "
                "falling back to simpler mode.",
                accuracy * 100,
            )
            items, remaining = await self._fix_incorrect_with_retries(
                items, pages, incorrect, start_index, max_attempts=1
            )
            ...
            logger.info(
                "Single fix pass insufficient (%.2f%%). Falling back.",
                post_accuracy * 100,
            )
        ...
        # Instead of crashing, return whatever partial results we have.
        # The downstream pipeline filters by physical_index != None and
        # builds the best tree possible from the available data.
        logger.warning(
            "All processing modes exhausted. Returning %d partial items "
            "(accuracy was %.2f%%).",
            len([i for i in items if i.get("physical_index") is not None]),
            accuracy * 100,
        )
        return items
    ...
    async def _process_toc_with_page_numbers(
        ...
        is_complete = await self._verify_toc_completeness(toc_content, toc_entries)
        if not is_complete:
            logger.warning(
                "TOC transformation incomplete (%d entries). Retrying once.",
                len(toc_entries),
            )
            async with self._sem:
            ...
            if len(retry_entries) >= len(toc_entries):
                ...
                logger.info(
                    "TOC transformation retry produced %d entries.", len(toc_entries)
                )
            else:
                logger.warning(
    ...
    async def _process_toc_no_page_numbers(
        ...
        if not is_complete:
            logger.warning(
                "TOC transformation incomplete (%d entries, no-page-numbers mode). "
                "Retrying once.",
                len(toc_entries),
            )
            async with self._sem:
    ...
    async def _fix_incorrect_with_retries(
        ...
        Mirrors v1's ``fix_incorrect_toc_with_retries`` behavior: keeps
        retrying until all incorrect items are resolved or *max_attempts*
        is reached.  Items that remain incorrect after all attempts have
        their ``physical_index`` set to ``None`` so they are filtered out
        downstream (graceful degradation rather than keeping wrong values).
        ...
        Example:
            >>> items, still_bad = await pi._fix_incorrect_with_retries(
            ...     items, pages, incorrect, start_index=1, max_attempts=3
            ... )
        """
        current_items = items
        ...
        # Graceful degradation: nullify physical_index on items that could
        # not be fixed after all attempts.  This ensures they are filtered
        # out by the existing ``physical_index is not None`` checks rather
        # than polluting the tree with incorrect page mappings.
        if current_incorrect:
            ...
            for idx in unfixable_indices:
                if 0 <= idx < len(current_items):
                    logger.warning(
                        "Nullifying physical_index for unfixable item '%s' "
                        "(list_index=%d, last attempted physical_index=%s)",
                        current_items[idx].get("title", "?"),
                        idx,
                        current_items[idx].get("physical_index"),
                    )
                    current_items[idx]["physical_index"] = None
    ...
    # ------------------------------------------------------------------
    # Internal: TOC completeness verification
    # ------------------------------------------------------------------

    async def _verify_toc_completeness(
        self,
        raw_toc: str,
        transformed_entries: list[dict[str, Any]],
    ) -> bool:
        """Check whether the transformed TOC is complete compared to the raw source.

        Uses the completeness checker agent to verify that no sections from
        the original TOC text were lost during transformation.  This mirrors
        v1's ``check_if_toc_transformation_is_complete`` behavior.

        Args:
            raw_toc: The original raw table of contents text.
            transformed_entries: The structured TOC entries after transformation.

        Returns:
            ``True`` if the transformed TOC is considered complete.

        Example:
            >>> is_complete = await self._verify_toc_completeness(
            ...     raw_toc="1. Intro ...\\n2. Methods ...",
            ...     transformed_entries=[{"structure": "1", "title": "Intro"}, ...],
            ... )
        """
        cfg = self.config
        agent = create_completeness_checker_agent(cfg.model)

        # Build a readable representation of the transformed entries
        transformed_repr = json.dumps(transformed_entries, indent=2)

        async with self._sem:
            result = await agent.run(
                completeness_check_prompt(raw_toc, transformed_repr)
            )
