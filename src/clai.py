...
from pydantic_ai.agent import AbstractAgent, Agent
...
"""The home directory for the AI CLI.
...
""",
    ...
    qualified_model_names = [
        n for n in get_literal_values(KnownModelName.__value__) if ":" in n
    ]
    ...
    parser.add_argument(
        "--no-stream", action="store_true", help="Disable streaming from the model"
    )
    ...
    name_version = f"[green]{prog_name} - AI CLI v0.1.0[/green]"
    if args.version:
    ...
    if args.agent:
        ...
        try:
            ...
            console.print(
                '[red]Error: Agent must be specified in "module:variable" format[/red]'
            )
            return 1
    ...
    if agent.model is None or model_arg_set:
        try:
            ...
            console.print(
                f"Error initializing [magenta]{args.model}[/magenta]:\n[red]{e}[/red]"
            )
            return 1
    ...
    if args.agent and model_arg_set:
        ...
        console.print(
            f"{name_version} using custom agent [magenta]{args.agent}[/magenta]",
            highlight=False,
        )
    else:
        console.print(
            f"{name_version} with [magenta]{model_name}[/magenta]", highlight=False
        )
        ...
        if ident_prompt.startswith("/"):
            ...
            try:
                messages = await ask_agent(
                    agent, text, stream, console, code_theme, deps, messages
                )
            ...
            async for node in agent_run:
                if Agent.is_model_request_node(node):
                    async with node.stream(agent_run.ctx) as handle_stream:
                        ...
                        stack.enter_context(
                            live
                        )  # entering multiple times is idempotent
