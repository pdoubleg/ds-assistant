...
def batch_documents(documents: list[Any]) -> list[list[Any]]:
    ...
    for document in documents:
        doc_chars = len(
            ...
            or getattr(document, "document_description", "")
            or getattr(document, "file_name", "")
        )
...
async def run_tag_batch(
    ...
    image_prompt_parts = mapper.build_image_prompt_parts(documents, runtime_storage)
    ...
    with batch_tagger_agent.override(instructions=instructions):
        result = await batch_tagger_agent.run(
            [prompt, *image_prompt_parts] if image_prompt_parts else prompt,
