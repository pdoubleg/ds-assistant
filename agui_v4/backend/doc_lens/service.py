...
class DocLensService:
    ...
    def ingest_pdf(
        ...
        for page_index in range(page_count):
            ...
            for asset in extracted_assets:
                ...
                x0, y0, x1, y1 = (
                    asset.bbox_norm if asset.bbox_norm is not None else (None, None, None, None)
                )
                pending_session_assets.append(
                    (
                        make_session_asset_row_id(
    ...
    def ingest_image(
        ...
        missing = self.db.get_missing_embedding_asset_hashes(session_id, self.embedder.model_key)
        num_new_embeddings = 0
        if asset_hash_val in missing:
            vec = self.embedder.embed_image(img_copy)
            self.db.bulk_insert_embeddings([(asset_hash_val, self.embedder.model_key, vec)])
            num_new_embeddings = 1
