...
# Error substrings that indicate DuckDB's internal state is unrecoverable
# on the current connection and a fresh connection is needed.
_FATAL_PATTERNS = (
...
def _is_fatal_duckdb_error(exc: Exception) -> bool:
    """Return True if `exc` is a known DuckDB fatal/invalidation error.

    Args:
        exc: The exception raised by a DuckDB operation.

    Returns:
        True when the error message matches a known fatal pattern.
    """
    msg = str(exc).lower()
    return any(p.lower() in msg for p in _FATAL_PATTERNS)
...
class DuckDBStore:
    ...
    def _reconnect(self) -> None:
        ...
        Called automatically when a fatal DuckDB error is detected. The old
        connection is closed best-effort; the new one re-runs schema init so
        tables and indexes are guaranteed to exist before the retry.
        """
        print("[DuckDBStore] Fatal error detected — reconnecting to database.", flush=True)
        try:
            self.conn.close()
        except Exception:
    ...
    def _execute(self, sql: str, params: list[Any] | None = None) -> duckdb.DuckDBPyConnection:
        ...
        Automatically reconnects once if a fatal DuckDB invalidation error is
        raised, then retries the statement on the fresh connection.

        Args:
            sql: SQL string to execute.
            params: Optional list of bound parameters.

        Returns:
            The cursor after execution (allows chained `.fetchone()` etc.).

        Raises:
            Exception: Re-raises any error that occurs after reconnection.
        """
        with self._lock:
            try:
                cursor = self.conn.cursor()
    ...
    def _executemany(self, sql: str, params: list[list[Any]]) -> None:
        ...
        Automatically reconnects once if a fatal DuckDB invalidation error is
        raised, then retries on the fresh connection.
    ...
    def bulk_insert_embeddings(self, rows: list[tuple[str, str, np.ndarray]]) -> None:
