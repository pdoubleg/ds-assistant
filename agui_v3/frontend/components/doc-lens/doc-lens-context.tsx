 ...
 *   - If the file set changes, the old backend session is deleted and a fresh
 *     one is started automatically, regardless of whether the overlay is open.
 *
 * Example usage:
 *   ```tsx
...
export function DocLensProvider({ docs, children, openLensRef }: DocLensProviderProps) {
  ...
  /** Start (or restart) a session with the current docs. */
  const initiateSession = useCallback(
    async (docList: UploadedDoc[]) => {
    ...
    [dl.sessionId, dl.clearSession, dl.startSession, currentDocKey]
  );

  // ── Effect: start session on first open ───────────────────────────────────
  // Runs whenever `open` flips to true and session is still idle.
  useEffect(() => {
    if (!open) return;
    ...
    // If already initialized or initializing, do nothing — state persists.
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [open]);

  // ── Effect: re-ingest when the doc set changes after first init ───────────
  // Runs whenever `docs` changes (file added/removed/replaced). Only acts if
  // a session has already been started so we don't fire before first open.
  useEffect(() => {
    if (!hasOpenedRef.current) return;              // not yet opened, ignore
    if (docs.length === 0) return;
    if (currentDocKey === lastIngestedKeyRef.current) return; // no change

    // Doc set has changed — restart the session so new files are indexed.
    initiateSession(docs);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [currentDocKey]);
