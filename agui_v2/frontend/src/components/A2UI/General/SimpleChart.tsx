    ...
    <svg width="100%" viewBox={`0 0 ${svgWidth} ${chartHeight + 40}`} className="overflow-visible">
      ...
      {points.map((p, i) => (
        <g key={i}>
          ...
          <text x={p.x} y={p.y - 10} textAnchor="middle" className="fill-foreground/70 text-[10px] font-medium">
            {values[i]}
          </text>
          <text x={p.x} y={chartHeight + 16} textAnchor="middle" className="fill-muted-foreground text-[10px]">
            {labels[i]}
          </text>
        </g>
      ))}
      ...
      <div className="space-y-1.5">
        {slices.map((slice, i) => (
          <div key={i} className="flex items-center gap-2 text-xs">
            ...
            <span className="text-foreground/80">{slice.label}</span>
