...
class AnalysisResult(BaseModel):
    ...
    Attributes:
        claim_overview: Narrative overview rendered as a TextBox.
        timeline_events: Chronological events rendered as a ClaimTimeline.
        summary_metrics: Key-value metrics rendered as a SummaryCard.
        findings: Observations rendered as FindingCard(s).
        tables: Tabular data rendered as DataTable(s).
        charts: Visualizations rendered as SimpleChart(s).

    Example:
        >>> result = AnalysisResult(
        ...     claim_overview="Wind damage claim filed 2025-03-10...",
        ...     summary_metrics=[SummaryMetric(label="Total", value="$12,450")],
        ...     findings=[Finding(title="Gap", content="...", severity="warning")],
        ... )
    """

    claim_overview: str = Field(
        ...,
        description=(
            "A concise narrative overview of the claim suitable for display "
            "in a summary text box. 2-4 sentences."
        ),
    )
    timeline_events: list[TimelineEvent] | None = Field(
        None,
        description=(
            "Chronological events in the claim lifecycle. Populate when the "
    ...
    summary_metrics: list[SummaryMetric] | None = Field(
        None,
        description=(
            "Key claim metrics at a glance (amounts, dates, statuses, parties). "
    ...
    findings: list[Finding] | None = Field(
        None,
        description=(
            "Observations, flags, or insights the auditor should be aware of. "
    ...
    tables: list[TableSpec] | None = Field(
        None,
        description=(
            "Structured tabular data such as estimate line items, coverage "
    ...
    charts: list[ChartSpec] | None = Field(
        None,
        description=(
            "Simple visualizations (bar, line, or pie) for numeric data. "
