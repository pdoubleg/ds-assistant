...
def create_demo_datasets():
    ...
    # Regression dataset
    X, y = make_regression(
        n_samples=500, n_features=8, n_informative=6, noise=0.1, random_state=42
    )
...
def run_cli_examples():
    ...
    print(
        "Command: python src/tune/tuner.py --data demo_classification.csv --target target --estimator xgboost --metric accuracy --max-iterations 2 --n-trials 20 --no-save"
    )

    # Example 2: Regression with config
    print("\n2. Regression with Custom Config:")
    print(
        "Command: python src/tune/tuner.py --config src/tune/config.yml --data demo_regression.csv --target target --estimator lightgbm --metric r2 --max-iterations 2 --no-save"
    )
    ...
    print(
        "Command: python src/tune/tuner.py --data demo_classification.csv --target target --estimator random_forest --metric f1 --max-iterations 2 --n-trials 15 --no-save"
    )
