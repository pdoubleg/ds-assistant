...
def exploratory_data_analysis(
    ...
    if numeric_cols and target in df.select_dtypes(include=[np.number]).columns:
        ...
        results["correlations"] = {
            ...
            "low_correlation_features": correlations[
                abs(correlations) < 0.1
            ].index.tolist(),
        }
    else:
        results["correlations"] = {
            "target_correlations": {},
            "note": "Limited correlation analysis (non-numeric target or no numeric features)",
        }
...
def format_eda_for_llm(
    ...
    lines = ["Dataset Analysis Summary:", "=" * 50, ""]
    ...
    if "target_correlations" in correlations and correlations["target_correlations"]:
        ...
        sorted_corrs = sorted(
            target_corrs.items(), key=lambda x: abs(x[1]), reverse=True
        )
        if sorted_corrs:
