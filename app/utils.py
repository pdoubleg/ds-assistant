...
def get_dummy_data(n_samples: int = 10000) -> pd.DataFrame:
    ...
    df["vehicle_type"] = np.random.choice(
        ["Sedan", "SUV", "Truck", "Coupe"], size=len(df)
    )
...
def get_dataset_summary_with_importance(
    ...
    lines = ["Dataset Summary Report", "=" * 50, ""]
    ...
    if df_test is not None:
        lines.append(
            f"- Test set shape: {df_test.shape[0]:,} rows × {df_test.shape[1]} columns"
        )
    ...
    if include_importance and model is not None:
        try:
            ...
            if hasattr(model, "feature_importances_"):
                ...
                lines.extend(
                    ["Feature Importance (mutual information):", "- Top features:"]
                )
            ...
            lines.extend(
                ["Feature Importance: Could not calculate", f"- Error: {str(e)}"]
            )
